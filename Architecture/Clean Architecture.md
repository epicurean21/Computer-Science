...
- 웹 어댑터와 애플리케이션 계층간에 서로 책임을 공유하지 않게하자.
- 아에 서로 모르게하자.
...
그래서, 언제 실제로 **육각형 아키텍쳐 (Hexagonal Architecture)**를 선택해야할까?  또는, 언제 전통적인 **계층형 아키텍처 (Layered Architecture)**를 고수해야할까?



### 도메인이 왕이다 (Domain is the King)

육각형 아키텍쳐의 주요 특징

- 외부 시스템에 대한 의존성 변화로부터 자유로운 상태로 도메인 코드를 개발할 수 있다.



**DDD** (Domain driven design) 개발을 집중적으로 하기 위한다면 ?

##### 육각형 아키텍쳐는 DDD의 조력자라고 할 수 있다.

도메인 코드가 애플리케이션에서 가장 중요한 것이 아니라면, 굳이 Hexagonal Architecture가 필요로하지 않다.



### 경험의 여왕이다

- 웹 애플리케이션을 개발한다면 계층형 아키텍처로 개발한다

습관에 의한 결정은 좋을수도, 나쁠수도있다. 우리가 편안함을 느끼고 있다면 굳이 새로움을 체택해야할까?

하지만 **새로운 경험이** <u>더 나은 결정을</u> 짓기위한 선택이 될 수 있다.



작은 프로젝트부터, 새롭게 배운 **Hexagonal Architecture**로 구현을 해보자.

- DDD에 집중된 개발을 경험하자.



### 그래서 언제 계층형 / 육각형 아키텍쳐를 선택할까 
...
- 스스로 웹 애플리케이션 개발 = Java/Spring and 계층형 아키텍처 라는 **습관**이 있다.

- DDD 패턴으로 개발을 시작하였지만, 거대한 서비스로 진화함에 따라 책에서 언급한데로 계층이 많이 무너졌다.
- MSA 환경으로 서비스를 분리함에 따라, 규모가 작아지고 도메인 주도 개발로 도메인 개발에 집중한다면 육각형 아키텍처를 채택하여 경험해 보는것도 좋은 방안이 될 것 같다.
